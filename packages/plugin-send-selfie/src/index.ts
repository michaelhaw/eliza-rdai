...
const sendSelfie: Action = {
    ...
    description: "Generate a selfie to go along with the message.",
    ...
    handler: async (
        ...
        //TODO: Create selfie profile based on agent characteristics
        //const CONTENT = message.content.text;
        const DEFAULT_SELFIE_PROFILE = "Selfie of a pretty young adult asian woman with a Gamer Girl aesthetic smiling and working on graphics design on her laptop in a cozy coffee shop during the day."
        const CONTENT = state.lore || DEFAULT_SELFIE_PROFILE;
        elizaLogger.log("Using the following for selfie profile:",CONTENT);
        ...
        const STYLE = "ghibli anime style";
...
Ensure that your prompt is detailed, vivid, and incorporates all the elements mentioned above while staying true to the content and the specified style. LIMIT the image prompt 50 words or less. \n\nWrite a prompt. Only include the prompt and nothing else.`;
        ...
        if (images.success && images.data && images.data.length > 0) {
            elizaLogger.log(
                "Selfie generation successful, number of images:",
                images.data.length
            );
            for (let i = 0; i < images.data.length; i++) {
                ...
                elizaLogger.log(`Processing selfie ${i + 1}:`, filename);
            ...
            elizaLogger.error("Selfie generation failed or returned no data.");
        }
    },
    examples: [
        ...
        [
            {
                user: "{{user1}}",
                content: { text: "Can you send me a selfie?" },
            },
            {
                user: "{{agentName}}",
                content: {
        ...
        [
            {
                user: "{{user1}}",
                content: { text: "I miss your smile" },
            },
            {
                user: "{{agentName}}",
                content: {
                    text: "Let me cheer you up with this!",
                    action: "SEND_SELFIE",
                },
            },
        ],
...
export const sendSelfiePlugin: Plugin = {
    name: "sendSelfie",
...
export default sendSelfiePlugin;
